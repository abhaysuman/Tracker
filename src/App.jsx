...
import BucketListPage from './BucketListPage'; // Include Bucket List
...
function App() {
  ...
  const [chatTarget, setChatTarget] = useState(null); // Opens chat with specific friend      
  ...
  useEffect(() => {
    if (userData?.theme && THEMES[userData.theme]) {
        ...
        // Inject CSS Variables to override default colors
        styleTag.innerHTML = `
            :root { 
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${isDarkMode ? 'dark' : ''}`}>
      ...
      {/* 2. PROTECTED PAGES (Wrapped in Sidebar Layout) */}
      ...
      <UserProfileModal 
        ...
        onMessageClick={(friendData) => { 
            setChatTarget(friendData); // Triggers chat in Sidebar
      ...
      {/* --- NO FLOATING BUTTONS HERE ANYMORE --- */}
    </div>
  );
}
