...
export default function Waveform({ audioUrl, stream, isMe, isRecording }) {
  ...
  useEffect(() => {
    ...
    // A: HANDLE RECORDING (Live Stream)
    ...
    // B: HANDLE PLAYBACK (Sent Message)
    else if (audioUrl) {
      if (wavesurfer.current) wavesurfer.current.destroy();

      wavesurfer.current = WaveSurfer.create({
        container: containerRef.current,
        waveColor: isMe ? '#fbcfe8' : '#cbd5e1', // Light Pink / Light Gray
        progressColor: isMe ? '#db2777' : '#64748b', // Dark Pink / Dark Gray
        cursorColor: 'transparent',
        barWidth: 3,
        barRadius: 3,
        cursorWidth: 0,
  ...
  if (isRecording) {
    // Just the waveform container for recording mode
