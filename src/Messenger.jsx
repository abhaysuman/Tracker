...
import { MessageCircle, ChevronDown, Minimize2, ChevronLeft, Plus, Video, Send } from 'lucide-react';
...
export default function Messenger({ isOpen, onClose, activeChatFriend, user, friends = [], onStartCall, onJoinCall }) {
  ...
  // Open chat if parent requests it
  useEffect(() => {
    if (activeChatFriend) {
      setIsExpanded(true);
  ...
  // 1. FETCH RECENT CHATS (Sorted in app to avoid Index error)
  useEffect(() => {
    if (!user) return;
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto bg-gray-50 dark:bg-black/20">
                ...
                {!activeChat && !showNewChat && (
                  <div className="p-2 space-y-1">
                    ...
                    {recentChats.length === 0 && <p className="text-center text-gray-400 text-xs mt-10">No chats yet.</p>}
                  </div>
                )}
                        ...
                        <div key={i} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                          {isCall ? (
                             ...
                             <div className={`max-w-[85%] p-3 rounded-2xl text-sm ${isMe ? 'bg-pink-100 border border-pink-200' : 'bg-white border border-gray-100 shadow-sm'}`}>
                               ...
                               <button 
                                 onClick={() => {
                                   if (isMe) onStartCall && onStartCall(msg.text); // Sender re-joins
                                   else onJoinCall && onJoinCall(msg.text);        // Receiver joins
                                 }}
              ...
              {activeChat && (
                <form onSubmit={sendMessage} className="p-2 bg-white dark:bg-midnight-card border-t border-gray-100 dark:border-white/5 flex gap-2 shrink-0">
                  ...
                  <button type="submit" disabled={!inputText.trim()} className="p-2 bg-pink-500 text-white rounded-full hover:bg-pink-600 disabled:opacity-50 transition-colors shadow-md">
                    <Send size={16} />
                  </button>
                </form>
              )}
