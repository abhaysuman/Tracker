...
// 1. ADD 'onJoinCall' TO PROPS
export default function Messenger({ isOpen, onClose, activeChatFriend, user, friends = [], onJoinCall }) {
  
  // ... (Keep all your existing state: isExpanded, activeChat, etc.) ...
  ...
  // ... (Keep your useEffects for fetching chats and messages exactly as they are) ...
  useEffect(() => {
      if (activeChatFriend) {
        setIsExpanded(true);
  ...
  // ... (Keep sendMessage, sendToFirebase, startNewChat exactly as they are) ...
    ...
    const startVideoCall = async () => {
      ...
      const roomId = `gf-mood-${user.uid}-${Date.now()}`; 
      const callUrl = `https://meet.jit.si/${roomId}#config.prejoinPageEnabled=false`; // Add config to skip prejoin
          ...
          {isExpanded && (
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto bg-gray-50 dark:bg-black/20">
                ...
                {/* (New Chat List - Same as before, code omitted for brevity but keep it in your file!) */}
                {!activeChat && showNewChat && (
                  <div className="p-2 space-y-1">
                    <div className="px-3 py-2 text-xs font-bold text-gray-400 uppercase tracking-wider">Select a Friend</div>
                    {friends.map(friend => (
                      <div key={friend.uid} onClick={() => startNewChat(friend)} className="flex items-center gap-3 p-3 hover:bg-white dark:hover:bg-white/5 rounded-xl cursor-pointer transition-colors">
                        ...
                        <div key={i} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                          {isCall ? (
                             ...
                             <div className={`max-w-[85%] p-3 rounded-2xl text-sm ${isMe ? 'bg-pink-100 border border-pink-200' : 'bg-white border border-gray-100 shadow-sm'}`}>
                               ...
                               <button 
                                 onClick={() => onJoinCall && onJoinCall(msg.text)} // <--- TRIGGERS MODAL
              ...
              {activeChat && (
                <form onSubmit={sendMessage} className="p-2 bg-white dark:bg-midnight-card border-t border-gray-100 dark:border-white/5 flex gap-2 shrink-0">
                  <input value={inputText} onChange={(e) => setInputText(e.target.value)} placeholder="Type a message..." className="flex-1 bg-gray-100 dark:bg-black/20 rounded-full px-4 py-2 text-sm outline-none dark:text-white focus:ring-2 focus:ring-pink-100 dark:focus:ring-white/10 transition-all" />
          ...
          <motion.button initial={{ scale: 0 }} animate={{ scale: 1 }} onClick={() => setIsExpanded(true)} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="w-80 bg-white dark:bg-midnight-card text-gray-700 dark:text-white p-3 rounded-t-xl shadow-[0_-5px_20px_rgba(0,0,0,0.1)] flex items-center justify-between border border-b-0 border-gray-100 dark:border-white/10 cursor-pointer">
            <div className="flex items-center gap-2">
              <div className="relative">
                <div className="w-8 h-8 rounded-full bg-pink-100 dark:bg-pink-900/30 flex items-center justify-center text-pink-500"><MessageCircle size={18} fill="currentColor" /></div>
