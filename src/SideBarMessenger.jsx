...
export default function SideBarMessenger({ user, userData, friends = [], activeChat, setActiveChat, onStartCall, onJoinCall, openDialog, onOpenSettings, onOpenProfile, onLogout, onNavigate, children }) {
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-[#1e1f22] font-sans">
      ...
      {/* 1. LEFT RAIL (Permanent) - MODIFIED BG COLOR */}
      ...
      <AnimatePresence mode="popLayout">
        {showSidebar && (
            <motion.div
                ...
                className="flex flex-col border-r border-gray-200 dark:border-[#1f2023] shrink-0 z-40 relative bg-[var(--theme-light)]/10 dark:bg-[#2b2d31]"
            >
                {/* Header */}
                <div className="h-12 border-b border-gray-200 dark:border-[#1f2023] flex items-center px-4 shadow-sm shrink-0">
                    <button className="w-full text-left bg-white dark:bg-[#1e1f22] text-gray-500 dark:text-[#949BA4] text-xs px-2 py-1.5 rounded flex items-center shadow-sm">
                ...
                <div className="flex-1 overflow-y-auto p-2 space-y-0.5 custom-scrollbar">
                    ...
                    {recentChats.map(chat => (
                        <div
                            ...
                            className={`flex items-center gap-3 px-2 py-2 rounded-lg cursor-pointer group transition-colors ${activeChat?.uid === chat.otherUser.uid ? 'bg-[var(--theme-primary)] text-white' : 'text-gray-600 dark:text-[#949BA4] hover:bg-black/5 dark:hover:bg-[#35373C]'}`}
                        >
                ...
                {/* User Widget */}
                <div className="mt-auto bg-white/50 dark:bg-[#232428] backdrop-blur-md border-t border-gray-200 dark:border-[#1f2023]">
                    <UserWidget
                        user={user}
                        userData={userData}
            ...
            <>
                <div className="h-12 border-b border-gray-100 dark:border-[#26272D] flex items-center justify-between px-4 shadow-sm bg-white dark:bg-[#313338] shrink-0">
                    ...
                    <div className="flex items-center gap-2 text-gray-400">
                        <button onClick={startVideoCall} className="p-2 hover:bg-gray-100 dark:hover:bg-white/10 rounded-full transition-colors"><Video size={20}/></button>
                            ...
                            <div key={i} className={`flex ${isMe ? 'justify-end' : 'justify-start'} group hover:bg-black/5 dark:hover:bg-[#2e3035]/30 -mx-4 px-4 py-1`}>
                                ...
                                <div className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} max-w-[75%]`}>
                                    {msg.type === 'image' ? <img src={msg.text} className="rounded-xl max-w-sm mb-1 border-2 border-[var(--theme-primary)]" /> :
                                     ...
                                     msg.type === 'call' ? <div className="bg-white dark:bg-[#2B2D31] p-3 rounded-xl border border-[var(--theme-primary)]/50 flex items-center gap-2"><Video size={16} className="text-[var(--theme-primary)]"/> <span className="text-gray-800 dark:text-white text-sm font-bold">Video Call</span></div> :
