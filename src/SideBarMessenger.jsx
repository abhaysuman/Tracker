...
import { MessageCircle, ChevronLeft, Plus, Video, Send, Mic, Trash2, MoreVertical, Search, X, Home, Calendar, BarChart2, Gift, CheckSquare, Phone, Users, CheckCheck, Image as ImageIcon } from 'lucide-react';
...
export default function SideBarMessenger({ user, userData, friends = [], activeChat, setActiveChat, onStartCall, onJoinCall, openDialog, onOpenSettings, onOpenProfile, onLogout, onNavigate, currentPage, children }) {
  // --- UI STATES ---
  const [showSidebar, setShowSidebar] = useState(false); // Sidebar hidden by default until "Messages" clicked
  ...
  // --- RENDER ---
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-gray-100 dark:bg-[#1e1f22] font-sans transition-colors duration-300">

      {/* 1. LEFT RAIL */}
      <div className="w-[72px] bg-gray-200/50 dark:bg-[#1e1f22] flex flex-col items-center py-4 gap-2 border-r border-gray-300 dark:border-[#1e1f22] shrink-0 z-50 shadow-sm">
        ...
        <NavIcon 
            ...
            onClick={() => { setActiveChat(null); setShowSidebar(false); onNavigate('home'); }} 
        />

        {/* MESSAGES (2nd) - Toggles Sidebar */}
        <NavIcon 
        ...
        <NavIcon 
            ...
            onClick={() => { setActiveChat(null); setShowSidebar(false); onNavigate('friends'); }} 
        />
        
        <div className="w-8 h-[2px] bg-gray-300 dark:bg-[#35363C] rounded-full mx-auto my-1"></div>
      ...
      <AnimatePresence mode="popLayout">
        {showSidebar && (
            <motion.div 
                ...
                className="flex flex-col border-r border-gray-200 dark:border-[#1f2023] shrink-0 z-40 relative bg-white dark:bg-[#2b2d31]"
            >
                <div className="h-12 border-b border-gray-200 dark:border-[#1f2023] flex items-center px-4 shadow-sm shrink-0">
                    <button className="w-full text-left bg-gray-100 dark:bg-[#1e1f22] text-gray-500 dark:text-[#949BA4] text-xs px-2 py-1.5 rounded flex items-center shadow-sm">
                ...
                <div className="flex-1 overflow-y-auto p-2 space-y-0.5 custom-scrollbar">
                    ...
                    {recentChats.map(chat => (
                        <div 
                            ...
                            className={`flex items-center gap-3 px-2 py-2 rounded-lg cursor-pointer group transition-colors ${activeChat?.uid === chat.otherUser.uid ? 'bg-[var(--theme-primary)] text-white' : 'text-gray-600 dark:text-[#949BA4] hover:bg-gray-100 dark:hover:bg-[#35373C]'}`}
                        >
                            <div className="relative">
                                <div className="w-9 h-9 rounded-full bg-gray-200 dark:bg-gray-500 overflow-hidden border border-white/10"><img src={chat.otherUser.avatar} className="w-full h-full object-cover" /></div>
                            </div>
                            <div className="flex-1 min-w-0">
                                <h4 className="font-bold text-sm truncate">{chat.otherUser.name}</h4>
                                <p className={`text-[11px] truncate h-3 ${activeChat?.uid === chat.otherUser.uid ? 'text-white/80' : 'text-gray-400'}`}>{chat.lastMessage}</p>
                            </div>
                        </div>
                    ))}
                </div>
                ...
                {/* USER WIDGET (Pinned) */}
                <div className="mt-auto bg-gray-50 dark:bg-[#232428] border-t border-gray-200 dark:border-[#1f2023]">
                    <UserWidget 
                        user={user} 
                        userData={userData} 
            ...
            <>
                <div className="h-12 border-b border-gray-100 dark:border-[#26272D] flex items-center justify-between px-4 shadow-sm bg-white dark:bg-[#313338] shrink-0">
                    <div className="flex items-center gap-3">
                        <div className="relative">
                            <div className="w-8 h-8 rounded-full overflow-hidden bg-gray-200 dark:bg-gray-500"><img src={activeChat.avatar || activeChat.photoURL} className="w-full h-full object-cover" /></div>
                            {isFriendOnline && <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 bg-green-500 rounded-full border-2 border-white dark:border-[#313338]"></div>}
                        </div>
                        <div>
                            <h3 className="font-bold text-gray-800 dark:text-white text-sm">{activeChat.name}</h3>
                            <p className="text-[10px] text-gray-500 font-bold">{isFriendTyping ? <span className="text-[var(--theme-primary)] animate-pulse">Typing...</span> : isFriendOnline ? "Online" : "Offline"}</p>
                        </div>
                    </div>
