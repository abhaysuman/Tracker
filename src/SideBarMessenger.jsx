...
export default function SideBarMessenger({ user, userData, friends = [], activeChat, setActiveChat, onStartCall, onJoinCall, openDialog, onOpenSettings, onOpenProfile, onLogout, onNavigate, children }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex bg-[#1e1f22] overflow-hidden font-sans">
      
      {/* 1. RAIL (Icons) */}
      <div className="w-[72px] bg-[#1e1f22] flex flex-col items-center py-3 gap-2 border-r border-[#1e1f22] shrink-0">
      ...
      <div className="w-60 bg-[#2b2d31] flex flex-col rounded-tl-xl border-r border-[#1f2023] shrink-0">
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-0.5">
            ...
            {recentChats.map(chat => (
                <div key={chat.id} onClick={() => setActiveChat(chat.otherUser)} className={`flex items-center gap-3 px-2 py-2 rounded-[4px] cursor-pointer group ${activeChat?.uid === chat.otherUser.uid ? 'bg-[#404249] text-white' : 'text-[#949BA4] hover:bg-[#35373C] hover:text-[#dbdee1]'}`}>
                    <div className="relative"><div className="w-8 h-8 rounded-full bg-gray-500 overflow-hidden"><img src={chat.otherUser.avatar} className="w-full h-full object-cover" /></div></div>
                    <div className="flex-1 min-w-0"><h4 className="font-medium text-sm truncate">{chat.otherUser.name}</h4><p className="text-[11px] opacity-70 truncate h-3">{chat.lastMessage}</p></div>
                </div>
            ))}
        </div>
        <UserWidget user={user} userData={userData} onOpenSettings={onOpenSettings} onOpenProfile={onOpenProfile} onLogout={onLogout} />
      </div>

      {/* 3. MAIN AREA (Chat OR Page) */}
      <div className="flex-1 bg-[#313338] flex flex-col min-w-0 relative">
        
        {/* IF CHAT IS ACTIVE -> SHOW CHAT */}
        {activeChat ? (
            <>
                {/* Chat Header */}
                ...
                <div className="px-4 pb-6 bg-[#313338] shrink-0">
                    <div className="bg-[#383A40] rounded-lg px-3 py-2 flex items-center gap-3">
                        ...
                        {isRecording ? (
                            <div className="flex-1 flex items-center gap-3 text-red-400 font-bold animate-pulse">
                                <Mic size={16} /> Recording... {formatTime(recordingTime)}
                                <button onClick={stopAndSendAudio} className="ml-auto text-xs bg-red-500 text-white px-2 py-1 rounded">Send</button>
                                <button onClick={cancelRecording} className="text-xs text-gray-400 hover:text-white">Cancel</button>
                            </div>
                        ) : (
                        ...
                        <div className="flex items-center gap-2 text-[#B5BAC1]">
                            <button onClick={startRecording} className="hover:text-white"><Mic size={20} /></button>
                            {inputText && <button onClick={sendMessage} className="text-pink-500 hover:text-pink-400"><Send size={20} /></button>}
                        </div>
                    </div>
                </div>
            </>
        ) : (
            // IF NO CHAT -> SHOW THE PAGE CONTENT (HOME, CALENDAR, ETC.)
