...
import { MessageCircle, ChevronLeft, Plus, Video, Send, Mic, Trash2, MoreVertical, Image as ImageIcon, CheckCheck, Search, X, Home, Calendar, BarChart2, Gift, CheckSquare, Phone } from 'lucide-react'; // Added Phone
...
export default function SideBarMessenger({ user, userData, friends = [], activeChat, setActiveChat, onStartCall, onJoinCall, openDialog, onOpenSettings, onOpenProfile, onLogout, onNavigate, children }) {
  ...
  // ... (Keep useEffects for fetching chats, messages, status) ...
  useEffect(() => { if (!user) return; const q = query(collection(db, "chats"), where("participants", "array-contains", user.uid)); const unsubscribe = onSnapshot(q, (snapshot) => { const chats = snapshot.docs.map(doc => { const data = doc.data(); const otherUser = data.users.find(u => u.uid !== user.uid); return { id: doc.id, ...data, otherUser }; }); chats.sort((a, b) => (b.lastUpdated?.toMillis() || 0) - (a.lastUpdated?.toMillis() || 0)); setRecentChats(chats); }); return () => unsubscribe(); }, [user]);
  useEffect(() => { if (!activeChat || !user) return; const chatId = [user.uid, activeChat.uid].sort().join("_"); const q = query(collection(db, "chats", chatId, "messages"), orderBy("createdAt", "asc")); const unsubscribeMsgs = onSnapshot(q, (snapshot) => { setMessages(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }))); setTimeout(() => scrollRef.current?.scrollIntoView({ behavior: "smooth" }), 100); snapshot.docs.forEach(async (docSnap) => { const msg = docSnap.data(); if (msg.senderId !== user.uid && !msg.read) await updateDoc(doc(db, "chats", chatId, "messages", docSnap.id), { read: true }); }); }); const unsubscribeChat = onSnapshot(doc(db, "chats", chatId), (docSnap) => setIsFriendTyping(docSnap.data()?.typing?.[activeChat.uid])); const unsubscribeUser = onSnapshot(doc(db, "users", activeChat.uid), (docSnap) => { if (docSnap.exists()) { const data = docSnap.data(); const presence = data.presence || 'online'; const isActiveNow = Date.now() - (data.lastActive?.toMillis() || 0) < 300000; setIsFriendOnline(presence !== 'invisible' && (presence !== 'online' || isActiveNow)); } }); return () => { unsubscribeMsgs(); unsubscribeChat(); unsubscribeUser(); }; }, [activeChat, user]);

  // ... (Keep existing helpers: handleInputChange, handleImageSelect, sendMessage, etc.) ...
