...
import CallInterface from './CallInterface'; // <--- IMPORT
import { motion, AnimatePresence } from 'framer-motion';

export default function SideBarMessenger({ user, userData, friends = [], activeChat, setActiveChat, onStartCall, onJoinCall, openDialog, onOpenSettings, onOpenProfile, onLogout, onNavigate, currentPage, activeCallProps, children }) {
  ...
  const handleStartVideoCall = async () => { if (!activeChat) return; const roomId = `call_${user.uid}_${Date.now()}`; if (onStartCall) onStartCall(roomId, 'video'); await sendToFirebase(roomId, 'call'); try { await addDoc(collection(db, "users", activeChat.uid, "notifications"), { type: 'call_invite', callType: 'video', message: 'is calling you! 🎥', roomId: roomId, senderUid: user.uid, senderName: user.displayName, timestamp: serverTimestamp(), read: false }); } catch (e) {} };
  const handleStartVoiceCall = async () => { if (!activeChat) return; const roomId = `call_${user.uid}_${Date.now()}`; if (onStartCall) onStartCall(roomId, 'voice'); await sendToFirebase(roomId, 'voice_call'); try { await addDoc(collection(db, "users", activeChat.uid, "notifications"), { type: 'call_invite', callType: 'voice', message: 'is calling you! 📞', roomId: roomId, senderUid: user.uid, senderName: user.displayName, timestamp: serverTimestamp(), read: false }); } catch (e) {} };
  ...
  // --- RENDER ---
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-[#EBD4F4] dark:bg-[#1e1f22] font-sans transition-colors duration-300">
      ...
      <div className="w-[72px] bg-white dark:bg-[#1e1f22] flex flex-col items-center py-4 gap-2 border-r border-gray-200 dark:border-[#1e1f22] shrink-0 z-50 shadow-sm">
        ...
        <NavIcon icon={<Users size={28} />} label="Friends" onClick={() => { setActiveChat(null); onNavigate('friends'); }} isActive={currentPage === 'friends'} />
        <div className="w-8 h-[2px] bg-gray-200 dark:bg-[#35363C] rounded-full mx-auto my-1"></div>
        <NavIcon icon={<Calendar size={24} />} label="Calendar" onClick={() => onNavigate('calendar')} isActive={currentPage === 'calendar'} />
        <NavIcon icon={<CheckSquare size={24} />} label="Bucket List" onClick={() => onNavigate('bucketlist')} isActive={currentPage === 'bucketlist'} />
        <NavIcon icon={<Gift size={24} />} label="Surprise" onClick={() => onNavigate('surprise')} isActive={currentPage === 'surprise'} />
        <NavIcon icon={<BarChart2 size={24} />} label="Insights" onClick={() => onNavigate('insights')} isActive={currentPage === 'insights'} />
      </div>

      {/* 2. SIDEBAR PANEL */}
      <AnimatePresence mode="popLayout">
        {showSidebar && (
            <motion.div initial={{ width: 0, opacity: 0 }} animate={{ width: 260, opacity: 1 }} exit={{ width: 0, opacity: 0 }} className="flex flex-col border-r border-gray-200 dark:border-[#1f2023] shrink-0 z-40 relative bg-gray-50 dark:bg-[#2b2d31]">
      ...
      <div className="flex-1 bg-white dark:bg-[#313338] flex flex-col min-w-0 relative h-full">
        {activeChat ? (
            <>
                {/* SPLIT SCREEN CALL */}
                {activeCallProps && (
                    <div className="h-[50%] shrink-0 border-b border-gray-900 bg-black relative">
                        <CallInterface 
                            callId={activeCallProps.callId} 
                            role={activeCallProps.role} 
                            user={user} 
                            activeChat={activeChat} 
                            callType={activeCallProps.callType} 
                            onClose={activeCallProps.onClose} 
                        />
                    </div>
                )}
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar bg-gray-50 dark:bg-[#313338]">
                    {messages.map((msg, i) => (
                        <div key={i} className={`flex ${msg.senderId === user.uid ? 'justify-end' : 'justify-start'} group hover:bg-black/5 dark:hover:bg-[#2e3035]/30 -mx-4 px-4 py-1`}>
                            {msg.senderId !== user.uid && <div className="w-8 h-8 rounded-full overflow-hidden mr-3 mt-1 bg-gray-300 dark:bg-gray-600"><img src={activeChat.avatar} className="w-full h-full object-cover" /></div>}
                            <div className={`flex flex-col ${msg.senderId === user.uid ? 'items-end' : 'items-start'} max-w-[75%]`}>
                                {msg.type === 'image' ? <img src={msg.text} className="rounded-xl max-w-sm mb-1 border-2 border-[var(--theme-primary)]" /> : 
                                 msg.type === 'audio' ? <div className="bg-white dark:bg-[#2B2D31] p-2 rounded-xl shadow-sm"><Waveform audioUrl={msg.text} isMe={msg.senderId === user.uid} /></div> : 
                                 msg.type === 'call' || msg.type === 'voice_call' ? <div className="bg-white dark:bg-[#2B2D31] p-3 rounded-xl border border-[var(--theme-primary)]/50 flex items-center gap-2"><Video size={16} className="text-[var(--theme-primary)]"/> <span className="text-gray-800 dark:text-white text-sm font-bold">Call Started</span><button onClick={() => onJoinCall(msg.text)} className="text-xs text-[var(--theme-primary)] font-bold hover:underline">Join</button></div> : 
                                 <div className={`text-[15px] leading-relaxed ${msg.senderId === user.uid ? 'text-gray-800 dark:text-gray-100' : 'text-gray-800 dark:text-gray-100'}`}>{msg.text}</div>}
                            </div>
                        </div>
                    ))}
                ...
                <div className="px-4 pb-6 bg-white dark:bg-[#313338] shrink-0">
                    <div className="bg-gray-100 dark:bg-[#383A40] rounded-2xl px-3 py-2 flex items-center gap-3 transition-all focus-within:ring-2 focus-within:ring-[var(--theme-primary)]/50">
                        ...
                        {isRecording ? <div className="flex-1 flex items-center gap-3 text-red-500 font-bold animate-pulse"><Mic size={18} /> Recording... {formatTime(recordingTime)}<button onClick={stopAndSendAudio} className="ml-auto text-xs bg-red-500 text-white px-3 py-1 rounded-full">Send</button><button onClick={cancelRecording} className="text-xs text-gray-400 hover:text-gray-600">Cancel</button></div> : <input value={inputText} onChange={handleInputChange} onKeyDown={(e) => e.key === 'Enter' && sendMessage(e)} placeholder={`Message @${activeChat.name}`} className="bg-transparent flex-1 outline-none text-gray-800 dark:text-[#DBDEE1] placeholder-gray-400" autoFocus />}
                        <div className="flex items-center gap-2 text-gray-400"><button onClick={startRecording} className="hover:text-[var(--theme-primary)]"><Mic size={20} /></button>{inputText && <button onClick={sendMessage} className="text-[var(--theme-primary)] hover:scale-110"><Send size={20} /></button>}</div>
                    </div>
                </div>
            </>
        ) : (
...
function NavIcon({ icon, label, onClick, isActive }) {
    return (
        <div className="group relative flex items-center justify-center w-[48px] h-[48px] cursor-pointer mb-2" onClick={onClick}>
            ...
            <div className="absolute left-16 bg-black text-white text-xs font-bold px-3 py-1.5 rounded shadow-xl opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none z-50">{label}</div>
        </div>
    )
}